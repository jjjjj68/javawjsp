	...
	<div class="text-center">
        ...
        <c:forEach var="preDay" begin="${preLastDay-(startWeek-2)}" end="${preLastDay}">
          ...
          <c:set var="cnt" value="${cnt+1}"/>
				...
				<%--   
				<!-- 시작일 이전을  공백처리한다.(오늘이 목요일이면 startWeek 5이기에, 1~4칸을 공백처리한다. -->
				...
				<c:forEach begin="1" end="${lastDay}" varStatus="st">
					<c:set var="todaySw" value="${toYear==yy && toMonth==mm && toDay==st.count ? 1 : 0}"/> <!-- 오늘날짜 -->
					<td id="td${cnt}" ${todaySw==1 ? 'class=today' : ''} style="font-size:0.9em"> <!-- 오늘날짜 -->
						${st.count}
					</td>
