...
<div class = "container">
	<form name="myform" method="post" action="${ctp}/boUpdateOk.bo">
		...
		<table class="table">
			...
			<tr>
				<th>이메일</th>
				<td><input type="text" name="email" value="${vo.email}" placeholder="이메일을 입력하세요" value="${email}" class="form-control"/></td>
			</tr>
			<tr>
				<th>홈페이지</th>
				<td><input type="text" name="homePage" value="${vo.homePage}" placeholder="홈페이지를 주소를 입력하세요" value="${homePage}" class="form-control"/></td>
			</tr>
			<tr>
				<th>글 내용</th>
			...
			<tr>
				<td colspan="2" class="text-center">
					...
					<input type="button" value="돌아가기" onclick="location.href='${ctp}/boContent.bo?idx=${vo.idx}&pag=${pag}&pageSize=${pageSize}';" class="btn btn-success"/>  &nbsp;
				</td>
			</tr>
		</table>
