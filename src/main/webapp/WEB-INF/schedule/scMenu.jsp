		...
		<script>
		    ...
		    function updateCheck(idx, part, content) {
				...
				function delCheck(idx) {
					let ans = confirm("선택된 일정을 삭제하시겠습니까?") /* 삭제하기전 물어보기 */
					if(!ans) return false;
					
					$.ajax({
						type  : "post",
						url   : "${ctp}/schedueDeleteOk.sc",
						data  : {idx : idx},
						success:function(res) {
							if(res == "1") {
								alert("일정이 삭제되엇습니다.");
  			...
  			<tr>
  				...
  				<td>
  					<a href="#" onclick="modalView('${vo.part}','${fn:replace(vo.content,newLine,'<br/>')}')"  data-toggle="modal" data-target="#myModal">
	  					...
	  					<c:if test="${fn:indexOf(vo.content,newLine) == -1}">${fn:substring(vo.content,0,20)}</c:if> <!-- 20자까지 가져오곘다  -->
  					</a>
  				</td>
  				<td>${vo.part}</td>
  				<td>
