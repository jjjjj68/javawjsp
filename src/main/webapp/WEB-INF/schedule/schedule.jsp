...
<div class = "container">
	<div class="text-center">
		<button type="button" onclick="location.href='${ctp}/schedule.sc?yy=${yy-1}&mm=${mm}';" class="btn btn-secondary btn-sm" title="이전년도">◁◁</button>
		<button type="button" onclick="location.href='${ctp}/schedule.sc?yy=${yy}&mm=${mm-1}';" class="btn btn-secondary btn-sm" title="이전월">◀</button>
		<font size="5">${yy}년${mm+1}월</font> <!-- mm을 11월 0으로 생각하기 때문에 +1 해주기 -->
		<button type="button" onclick="location.href='${ctp}/schedule.sc?yy=${yy}&mm=${mm+1}';" class="btn btn-secondary btn-sm" title="다음월">▶</button>
		<button type="button" onclick="location.href='${ctp}/schedule.sc?yy=${yy+1}&mm=${mm}';" class="btn btn-secondary btn-sm" title="다음년도">▷▷</button>
		&nbsp; &nbsp;
	...
	<div class="text-center">
        ...
        <c:forEach var="preDay" begin="${preLastDay-(startWeek-2)}" end="${preLastDay}">
          ...
          <c:set var="cnt" value="${cnt+1}"/>
				...
				<!-- 해당월에 대한 날짜를 마지막일자까지 반복 출력한다. (단, gpa이 7이되면 줄바꿈한다.) -->
