...
create table member (
	...
	pwd varchar(100) not null,					/* 회원 비밀번호(SHA암호화 처리) 암호화하면 길이가 길어져서 varchar 값 크게하기 */
	...
	job				varchar(20),							/* 회원직업 */
	homePage varchar(50) not null,			/* 홈페이지 */
	hobby			varchar(20),							/* 회원취미(2개이상이면 '/'로 구분처리) */
	photo			varchar(100) default 'noimage.jpg', /* 회원사진 */
	content text,												/* 회원자기소개 */
	userInfor char(6) default '공개',		/* 회원정보 공개부여(공개/비공개) */
	userDel		char(2) default 'NO',	    /* 회원 탈퇴 신청 여부(OK:탈퇴신청한회원, NO:현재가입중인회원) */
	point			int default 100,					/* 회원누적포인트(가입시는 기본 100증정, 방문지마다 1회 10포인트 증거, 최대 1일 50포인트까지) */
