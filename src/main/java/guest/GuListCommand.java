...
public class GuListCommand implements GuestInterface {
	/*
	 페이징 처리
	 1. 페이지(pag)를 결정한다. 처음 접속시의 페이지는 무조건 1page 이다. : pag :1
	 2. 한 페이지의 분량을 결정한다. : pageSize = 5(사용자가 결정한다) 이곳에서는 한페이지분량을 5로 했다. 
	 3. 총 레코드 건수를 구한다. totRecCnt => SQL함수중 count(*)을 사용한다.
	 4. 총 페이지 건수를 구한다. totPage => totRecCnt % pageSize 값이 0 이면 '몫' , 0이 아니면 '몫+1'
	 5. 현재 페이지의 시작 인덱스 번호를 구한다. startIndexNO => (pag - 1) * pageSize
	 6. 현재 화면에 보여주는 시작 번호를 구한다. curScrStarNo => totRecCnt - startIndexNO
	  */
...
//		System.out.println("totRecCnt :" + totRecCnt); 인덱스번호 확인
		...
		// 블록 페이징 처리..........(3단계) -> 블록의 시작번호를 0번부터 처리했다.
